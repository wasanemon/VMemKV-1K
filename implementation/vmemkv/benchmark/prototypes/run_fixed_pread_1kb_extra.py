#!/usr/bin/env python3
"""固定preadの追加5条件。測定済みバイナリを再利用し、各版2回だけ比較。"""
import hashlib
import json
import os
...
r = base.runner
OUT = base.OUT / 'extra-20260923'
DATA = base.ROOT / 'build/ltm/data/fixed-pread-extra-20260923'
...
def main():
    ...
    cells.append(('ltm', 1024, 'GetUpdate50', 'Zipf', 1))
    r.save(OUT / 'plan.json', {'cells': cells, 'repetitions': 2, 'total_runs': 20,
        ...
        'branch_at_start': subprocess.check_output(['git', 'branch', '--show-current'], text=True).strip(),
        'note': '固定preadの既存バイナリ。混在は毎回専用checkpointコピーへ戻す。'})
    r.save(OUT / 'datasets.json', {'1024': {'path': str(base.DB), 'count': base.COUNT, 't2_bytes': 7030530024}})
    r.OUT, r.DATA = OUT, DATA
    for cell in cells:
        for rep in [1, 2]:
            for variant in (base.VARIANTS if rep == 1 else base.VARIANTS[::-1]):
    ...
    for cell in cells:
        raw = {v: [json.loads((OUT / f'r{rep}-{r.label(cell)}-{v}-pinned/throughput.json').read_text())['ops_per_second']
                   for rep in [1, 2]] for v in base.VARIANTS}
        a, b = (statistics.mean(raw[v]) for v in base.VARIANTS)
        rows.append(dict(zip(['scenario', 'value_bytes', 'operation', 'distribution', 'threads'], cell),
                         proposal1=a, fixed_pread=b, change_percent=(b/a-1)*100, raw=raw))
